...
import jwt
from argon2 import PasswordHasher, exceptions   # ← NEW import
from ..models.user import User
from ..config import settings

logger = logging.getLogger(__name__)

# Password hashing configuration using Argon2 (modern replacement for bcrypt)
# No 72-byte limit, better security against GPU attacks
# Parameters roughly equivalent to bcrypt rounds=12
ph = PasswordHasher(
    time_cost=2,          # iterations (higher = slower, more secure)
    memory_cost=65536,    # 64 MiB memory (memory-hard)
...
class AuthService:
    ...
    def create_user(
        ...
        Security Notes:
            - Password is hashed using Argon2 (no length limit)
        ...
        # [Task]: T019 - Hash password with Argon2
        # No truncation needed – Argon2 handles any length
        ...
        user = User(
            id=user_id,
            email=email,
            password_hash=password_hash,  # ← field name same rakha (adjust if needed)
            name=name,
    ...
    def verify_password(session: Session, email: str, password: str) -> Optional[User]:
        ...
        # [Task]: T020 - Verify password hash using Argon2
...
#         # Password length is validated at schema level (max 72 chars/bytes)
